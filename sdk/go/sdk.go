...
// flush serializes the span and sends it to the gateway over gRPC.
// NOTE: This requires the proto package generated by scripts/gen-proto.sh.
// Until that runs, replace the body with the import and pb.NewIngestionServiceClient call:
//
//	pbClient := pb.NewIngestionServiceClient(c.conn)
//	resp, err := pbClient.IngestSpan(ctx, &pb.IngestSpanRequest{Span: spanToProto(s, endTime)})
func (c *Client) flush(_ context.Context, _ *Span, _ time.Time) error {
	// TODO: uncomment after running scripts/gen-proto.sh
	return nil
}
