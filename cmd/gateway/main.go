...
func main() {
	...
	// NOTE: RegisterIngestionServiceServer is generated by gen-proto.sh.
	// Until protos are generated, this line will not compile — run scripts/gen-proto.sh first.
	handler, err := NewIngestionHandler(cfg, logger)
	if err != nil {
		logger.Error("failed to init ingestion handler", "err", err)
		os.Exit(1)
	}
	RegisterIngestionServiceServer(srv, handler)
